...
class MultiHeadTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        def weighted_mse(targets, predictions):
            weights = torch.Tensor(list(map(lambda x: 1 / x if x != 0 else 0, targets))).to(device)
        ...
        for batch_idx in range(targets_batch.size()[0]):
            ...
            loss = weighted_mse(targets, predictions)
            losses.append(loss)
...
def main(
    ...
    if human_model == "human":
        output_dir = Path(__file__).parent.parent.joinpath("model", f"{dataset}_{human_model}_{base_model}_labels_dist_regression_weighted_finetuned")
