...
class MultiHeadTrainer(Trainer):
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        def loss_fct(targets, predictions, constraints):
            # print(f"targets size: {targets.size()}")
            # print(f"targets: {targets}")
            # Apply softmax to convert predictions to probabilities
            predictions_probs = F.softmax(predictions, dim=1)
            # Apply constraints to ensure the sum of counts is 5 for each instance
            # counts = constraints.sum()
            # normalized_predictions = predictions_probs * counts
            # print(f"normalized_predictions size: {normalized_predictions.size()}")
            # print(f"normalized_predictions[0,0,0]: {normalized_predictions[0, 0, 0]}")
            # loss = F.nll_loss(torch.log(normalized_predictions), targets)
            loss = F.nll_loss(torch.log(predictions_probs), targets)
            return loss
            
        losses = [loss_fct(targets_batch[:, idx], predictions, targets_batch[0, :]) for idx, predictions in enumerate(predictions_per_head)]
...
def main(
    ...
    if human_model == "human":
        output_dir = Path(__file__).parent.parent.joinpath("model", f"{dataset}_{human_model}_{base_model}_labels_dist_finetuned")
