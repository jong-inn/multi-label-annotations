...
def main(
    ...
    if weighted:
        prediction_human = pd.read_csv(prediction_path.joinpath(f"{dataset}_human_{base_model}_labels_dist_regression_weighted_prediction.csv"))
        prediction_model = pd.read_csv(prediction_path.joinpath(f"{dataset}_model_{base_model}_labels_dist_regression_weighted_prediction.csv"))        
    else:
        prediction_human = pd.read_csv(prediction_path.joinpath(f"{dataset}_human_{base_model}_labels_dist_regression_prediction.csv"))
        prediction_model = pd.read_csv(prediction_path.joinpath(f"{dataset}_model_{base_model}_labels_dist_regression_prediction.csv"))
    
    
    prediction_human["prediction"] = prediction_human["prediction"].str.strip("[]").str.split(",").apply(lambda x: [int(round(float(i.strip()))) for i in x])
    prediction_model["prediction"] = prediction_model["prediction"].str.strip("[]").str.split(",").apply(lambda x: [int(round(float(i.strip()))) for i in x])
    ...
    if plot_type == "displot":
        ...
        if weighted:
            plt.savefig(Path(__file__).parent.parent.joinpath("plot", f"{dataset}_{base_model}_{plot_type}_labels_dist_regression_weighted_exploded.pdf"), format="pdf")
        else:
            plt.savefig(Path(__file__).parent.parent.joinpath("plot", f"{dataset}_{base_model}_{plot_type}_labels_dist_regression_exploded.pdf"), format="pdf")
        ...
        if weighted:
            plt.savefig(Path(__file__).parent.parent.joinpath("plot", f"{dataset}_{base_model}_{plot_type}_human_head_labels_dist_regression_weighted.pdf"), format="pdf")
        else:
            plt.savefig(Path(__file__).parent.parent.joinpath("plot", f"{dataset}_{base_model}_{plot_type}_human_head_labels_dist_regression.pdf"), format="pdf")
        ...
        if weighted:
            plt.savefig(Path(__file__).parent.parent.joinpath("plot", f"{dataset}_{base_model}_{plot_type}_model_head_labels_dist_regression_weighted.pdf"), format="pdf")
        else:
            plt.savefig(Path(__file__).parent.parent.joinpath("plot", f"{dataset}_{base_model}_{plot_type}_model_head_labels_dist_regression.pdf"), format="pdf")
